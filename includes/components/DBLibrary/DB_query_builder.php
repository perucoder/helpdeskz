<?php
namespace HelpDeskZ\Components\DBLibrary;
/**
 * Query Builder Class
 *
...
abstract class DB_query_builder extends DB_driver {
    ...
    public function paginate($per_page = null, $sorts=null, $type='object')
    {
        if($per_page == '')
        {
            $per_page = 25;
        ...
        $page_var = 'p';
        $total_rows = $this->count_all_results('',false);
        ...
        if($total_rows > 0)
        {
            if(is_array($sorts))
            {
                ...
                {
                    if(array_key_exists($_GET[$filter_sort], $sorts))
                    {
                        $this->qb_orderby = array();
                        $this->qb_cache_orderby = array();
                        $this->order_by($sorts[$_GET[$filter_sort]], $_GET[$filter_order] == 'desc' ? 'desc' : 'asc');
            ...
            $current_page = isset($_GET[$page_var]) && $_GET[$page_var] > 0 ? (int) $_GET[$page_var] : 1;
            $current_page = ($current_page > $total_pages) ? $total_pages : $current_page;
            $offset = ($per_page * $current_page) - $per_page;

            $q = $this->limit($per_page, $offset)->get();
            if($type == 'object')
            {
                $r = $q->result();
            }else{
